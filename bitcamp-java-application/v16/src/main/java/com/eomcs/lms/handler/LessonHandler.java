...
public class LessonHandler { // 분리: 핸들러는 입출력만, 데이터 보관은 LessonList클래스에서
  ...
  private LessonList lessonList = new LessonList(); // 핸들러는 저장하는 일을 List클래스로 넘겼으니까, List를 쓰기위해서 여기서
  ...
  public void addLesson() {
    ...
    lessonList.add(lesson); // lesson 수업 주소를 주면서 LessonList클래스에서 파라미터로 받은 lesson로 배열에 추가한다.
    System.out.println("저장하였습니다.");
  }


  public void listLesson() {
    Lesson[] lessons = lessonList.toArray(); // lessonList야~ toArray에 들어있는 데이터 배열에 있는 값 리턴해줘
