...
public class PlatformTransactionManager {
  ...
  private SqlSessionFactory sqlSessionFactory; //proxy에 담긴 Session이 담긴다.
  ...
  public void beginTransaction() throws Exception {
    // 여기에서 스레드가 사용할 SqlSession 객체를 미리 준비한다.  ====> 스레드주머니에 보관
   ((SqlSessionFactoryProxy)sqlSessionFactory).prepareSessionInThread();
  }

  public void commit() throws Exception {
    // 현재 스레드의 주머니에서 SqlSession을 꺼낸다.
    SqlSession sqlSession = sqlSessionFactory.openSession();            //localSqlSession의 Session get()
    sqlSession.commit();
