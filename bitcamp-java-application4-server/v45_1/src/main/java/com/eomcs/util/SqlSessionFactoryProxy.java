...
public class SqlSessionFactoryProxy implements SqlSessionFactory {
  ...
  // => 스레드 풀을 사용할 때는 스레드가 작업을 응답을 완료한 후 재사용 되기 때문에, 
  //    다른 클라이언트의 요청을 처리하기 위해 재사용되기 전에 
  //    이전에 사용했던 SqlSession을 제거해야 한다.
  // 왜?
  //    이전에 사용한 SqlSession은 트랜잭션을 종료하는 순간 close()되었기 때문이다.
  ...
  public SqlSession openSession() {
    ...
    //스레드의 SqlSession이 들어 있으면(트랜잭션이 시작된 상태) 그것을 그대로 리턴한다.
    if(sqlSession != null) 
      return sqlSession;

    // 트랜잭션을 수행하지 않는 상태라면, 자돝커밋을 수행하는 SqlSession을 리턴한다.
