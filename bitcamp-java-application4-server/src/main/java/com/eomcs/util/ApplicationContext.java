...
public class ApplicationContext {
  ...
  public ApplicationContext(String packageName) throws Exception {
    ...
    // => 패키지의 경로를 알아낸다.
    String packagePath = packageName.replace(".", "/"); //패키지 이름인 com.eomcs.lms 이것을 경로인 /로 바꾸기
    File fullPath = Resources.getResourceAsFile(packagePath); //패키지의 실제 경로 리턴해줌
    ...
    keySet.forEach(key -> {
      System.out.printf("%s : %s\n", 
          key, objPool.get(key).getClass().getName()); //objPool.으로 키를 꺼내고 객체타입.이름을 출력하기
    });
  }
    ...
    for (File f : files) {
      if (f.isDirectory()) {
        ...
        try {
          ...
          if (isCommand(clazz) && !Modifier.isAbstract(clazz.getModifiers())) { //isCommand: 커맨드 인터페이스 구현했고이고 추상클래스가 아니면 ==>일반클래스면 clazz저장
            classes.add(clazz);
          }
  ...
  private void createCommand() {
    for (Class<?> clazz : classes) { //classes : command 구현체들
  ...
  //파라미터에 값을 주면 값을 배열에 담고 그 파라미터.getType() 타입 목록을 리턴한다
