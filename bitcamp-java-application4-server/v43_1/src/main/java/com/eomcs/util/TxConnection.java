...
//프록시 패턴?
// =>다른 객체에서 원래 객체인 것 처럼 보여야 하기 때문에 원래의 객체와 같은 인터페이스를 구현한다.
// =>그러나 실제 일을 할 때는 자신이 하는 것이 아니라 원래 객체에게 떠넘긴다. (위임)
// => 이떄 위임하기 전에 추가하거나 변경하고 싶은 작업이 있다면 처리하면 되는 것이다.
...
public class TxConnection implements Connection {
  ...
  //기존 메서드 중에서 close()에 대해서는 변경 작업을 추가한다.
  public void close() throws SQLException {
    
    // close()를 호출하더라도 닫지 않는다.
    // realClose()를 호출할 때 진짜 닫는다.
