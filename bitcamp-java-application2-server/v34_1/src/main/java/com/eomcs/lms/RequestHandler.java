...
public class RequestHandler extends Thread {
  ...
  public static boolean isStopping = false; // 기본은 멈추면 안된다.

  Socket socket;
  Map<String, Object> servletContext;

  // 클라이언트가 접속을 하면 그 소켓정보를 가지고 이 클래스의 인스턴스를 생성한다.
  ...
  // 별도의 실행흐름(실; 스레드)은 run()메소드에 작성한다.
  @Override
  public void run() {
    ...
    try (Socket clientSocket = this.socket;
      ...
      } else if ((servlet = findServlet(command)) != null) { // null이 아니다? 명령어를 처리할 서블릿을 찾았다.
