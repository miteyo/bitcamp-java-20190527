...
- DB 모델링? 데이터를 분석하고 구조화시켜 데티어  속성과 관계를 글과 그림으로 표현한 것.
...
### 01. 엔티티(테이블, Relation) 식별 및 속성 식별
...
#### 03. 제1정규화 p.403 
- 정규화? 데이터 중복을 찾아내어 별도의 테이블로 데이터를 분리시키는 것. (중복데이터를 제거하는 작업)
- 중복 데이터 또는 중복 컬럼을 별도의 테이블로 분리하여 부모-자식 관계를 맺는다.
- 데이터를 참조 하는 테이블이 자식테이블이고, 데이터를 갖고있는 테이블이 부모 테이블이다.
- 자식 테이블에서는 부모테이블의 데이터를 가리키기 위해 그 데이터의 PK 값을 보관해야 한다.
...
- 비슷한 종류의 컬럼들을 묶어 새 타입으로 정의한다. (예: varchar(50) -> varchar(25)) 
...
- 참고: 역 정규화?
-    =>데이터가 실행속도를 높이기 위해 데이터 중복을 허용하는 것
