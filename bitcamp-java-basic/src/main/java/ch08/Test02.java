...
public class Test02 {
  public static void main(String[] args) {
    //클래스에 대한 레퍼런스 변수 만들 때는 클래스를 로딩하지 않는다! 않는다! 않는다! 
    My2 temp;
    
    Scanner keyScan = new Scanner(System.in);
    System.out.println("> ");
    keyScan.nextLine(); //사용자가 입력한 값 중에서  Line Feed (LF; 0x0A)코드를 읽을때 까지 리턴하지 않는다.
    
    // 다음과 같이 My2를 사용하는 시점에 My2.class 파일을 Method Area 영역에 로딩한다.
    // 이것을 Test 하려면 이 클래스를 실행하기 전에 My2.class 파일을 삭제하라. 그리고 이 클래스를 실행하면,
    // 위의 코드를 사용할때는 오류가 발생하지 않다가, 아래의 코드를 실행 할 떄, My2.class가 없다고 오류가 발생한다.(메모리 낭비 하지 않으려고, 필요한 시점에 메모리를 사용하기 위해서)
    ...
    //    - My2의 클래스 필드(static field)를 "Method Area"에 생성한다.
