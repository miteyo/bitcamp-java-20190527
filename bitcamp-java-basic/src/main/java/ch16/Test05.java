...
public class Test05 {
  public static void main(String[] args) {
    ...
    System.out.println(strs2.length); // 100개의 배열을 넘겼는데 10개의 배열만 만들어진다.
    System.out.println(temp == strs2);

    // 생성할 배열의 타입 정보를 넘긴다.
    String[] strs3 = create3(String.class); // 모든 클래스에는 내장 변수로 class정보 변수가 있다.(클래스이름.변수명)
  ...
  static <T> T[] create1() {
    // T[] arr = new T[10]; // 컴파일 오류! new 명령어를 사용할 떄 제네릭의 타입 파라미터를 사용 할 수 없다.
    return null;
  }
  ...
  static <T> T[] create3(Class<?> type) { // (클래스 정보를 받아서 ) 그 클래스 타입에 해당하는 배열을 10개 만든다.
