...
public class Test01_2 {
  public static void main(String[] args) throws Exception {
    ...
    byte[] buf = new byte[8192]; //실무에서는 보통 8KB=8192byte
    int len = 0;
    int count = 0;
    while ((len = in.read(buf)) != -1) {
      count++;
      // 파일에서 읽은 1바이트를 암호화 한다고 가정해보자!
      // 바이트 배열에 데이터를 읽어 왔으면 다음과 같이 바이트 배열을  반복하여 특정 작업을 수행 해야 한다.
      for(int i = 0; i < len; i++) {
        // 각 바이트를 암호화 시키는 코드를 여기에 작성 해야 한다.
