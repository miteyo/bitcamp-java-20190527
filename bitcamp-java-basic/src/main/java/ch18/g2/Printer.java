...
public interface Printer {
  ...
  // 일반적으로 선언하는 인터페이스의 메소드는 public abstract 메소드이다.
  // 따라서  이 인터페이스를 구현하는 모든 클래스는  반드시 이 메소드를  정의해야 한다.
  void print(String text);

  //다음과 같이 새 기능을 무심코 추가한다면 기존에 이 인터페이스 규칙에 따라 작성한 모든 클래스들에 컴파일 오류가 발생할 것이다.
  //왜? 새 규칙(메서드)가 추가되었기 때문이다.
  //새 규칙이 추가 되었으면 기존 클래스들도 새규칙을 구현 해야한다.
  ...
  // 이 메서드를 추가 했다고 해서, 기존에 작성한 클래스(PaterPrinter, FilmPrinter)를 변경할 필요는 없다.
  // 컴파일 오류 발생하지 않는다.
}
