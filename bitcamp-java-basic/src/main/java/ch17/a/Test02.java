...
public class Test02 {
  ...
  public static void main(String[] args) {
    ...
    A obj4 = new My2();
    obj4.m1(); // m1()은 A클래스에 있는 메서드이다. 호출가능.
               // 다만 obj4에 저장된 인스턴스 주소는 실제 My2 클래스이기 때문에 m1() 메서드를 찾을떄는
               // My2 클래스부터 찾아 상위 클래스로 찾아 올라간다.

    obj4.m2("임꺽정"); // m2() 또한 m1() 과 같다. A클래스 메소드라 호출 가능. 
                     //다만, 인스턴스 주소는 My2 클래스이기 때문에 My2의 m2() 이다.
                     //컴파일 할 때는 문법이 맞는지만 -> A클래스에 m2()가 있는지 없는지만 따진다.

    //obj4.m3("임꺽정"); //컴파일 오류!
                       //obj4는 A클래스의 레퍼런스이기 때문에 문법적으로 m3() 메소드가 없기 때문에 호출 불가.
                       //obj4가 어떤 클래스의 인스턴스를 가리키는 가는 중요한 것이 아니다.
                       // 일단 메소드를 호출할 때 레퍼런스의 클래스에 없는 메소드는 호출할 수 없다.
    
    //obj4에 My2 인스턴스를 저장한 것이 확실하다면 컴파일러에게 개발자가 직접 알려줘야 한다.
    // 즉 다음과 같이 해당 레퍼런스에 어떤 클래스의 인스턴스가 들어있는지
    // 타입캐스팅(형변환) 해 줘야 한다. 
