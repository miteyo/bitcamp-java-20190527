...
public class Test02 {
  ...
  public static void main(String[] args) {
    ...
    // => 그런데 이 클래스를 사용하는 개발자 쪽에서는  Sorter를 상속받았으니,
    //    당연히 정렬을 수행하기 위해 sort() 메서드르 호출 할 것이다.
  ...
  static void display(Sorter sorter, int[] values) {
    ...
    //당연히 파라미터로 넘어오는 클래스는 Sorter의 서브클래스일 것이다.
    // Sorter는 추상 클래스이기 때문에 인스턴스를 생성할 수 없다.
    // 따라서 Sorter의 인스턴스가 넘어올 수는 없는 것이다. 
    
    // 따라서 정렬을 수행하려면 Sorter에 정의된 sort()를 호출 할 것이다.
    // 여기서 문제가 발생한다.
    // BubbleSort나 QuickSort 클래스는 sort()를 오버라이딩 하였다.
    // 그러나 MergeSort는 sort()을 오버라이딩 하지 않았.
    // 따라서 파라미터로 넘어오는 객체가 MergeSort라면 다음 메서드를 호출하는 것은 아무런 의미가 없다.
