// hash code 응용 III - Wrapper 클래스를 key객체로 사용하기
package ch15;
...
public class Test10_1 {
  public static void main(String[] args) {
    ...
    map.put(k1, new Student("홍길동", 20, false)); // (k1, 힙에 저장된 Student 가 놓인다)
    map.put(k2, new Student("임꺽정", 30, true));
    map.put(k3, new Student("유관순", 17, true));

    // (해시코드를 갖고있음 , 실제 저장할 객체)
    // put(Object key, Object value) (주소, 주소)
    // => put() 메서드는 key로 넘겨받은 객체에 대해 hashCode()를 호출하여 정수 값을 얻는다.
    // => 그렇게 리턴 받은 정수값(해시코드)를 사용하여 Student 객체(주소)를 저장할 위치를 계산한다.
    // => 그런 후 그 위치에 해당하는 배열(배열로 관리한다면)에 저장한다.

    // 다음과 같이 int를 key로 사용할 수 있다.
    // => key 값으로 int를 넘겨준다면, 내부적으로 auto-boxing을 수행하여 Integer 객체를 만든다.
    // 그리고 그 객체를 넘겨주는 것이다.

    // 원래는 (주소, 주소) ->(int, 주소) -> 오토박싱
    ...
    // 결론!
    // => k2와 k6는 다른 객체지만,
     //hash코드의 리턴값이 같고 equals()의 리턴값이 true이기 때문에 두 객체는 같은 key로 간주된다.
    ...
    // Object 클래스에서 상속받은 hashCode()는 인스턴스 필드의 값이 같은지 따지지 않고 무조건 인스턴스마다 고유의 해시값을 리턴한다.
    // 또한 equals()는 인스턴스 필드의 값이 같은지 비교 하지 않고, 같은 인스턴스인지만 따진다.
     
     // 그러나 Integer클래스 등의  Wrapper클래스와 String 클래스는 
     // Object 클래스에서 상속받은 hashCode()와 equals() 를 오버라이딩 했기 때문에
     // 인스턴스가 다르더라도 인스턴스 필드의 값이 같으면 hashCode()의 리턴값이 같고, equals()가 true를 리턴한다.
     // ==> 그래서 Wrapper클래스와 String클래스 HashMap/HashTable의 key로 사용할 수 있는 것이다.
     //
     // 실무에서도 String이나 PrimitiveType (결국 wrapper 객체로 전환되기 때문에)를 key로 자주 사용한다.
     // 
  }
